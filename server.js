...
app.post("/api/createUser/mail", (req, res) => {
  const { personalEmail, email, password } = req.body; // using req.body
  console.log('Sending email to:', personalEmail);
  ...
  let mailOptions = {
    ...
    to: personalEmail, // No need for interpolation if it's a string already
...
app.get("/api/getUser", async (req, res) => {
  try {
    const users = await User.find({}, { personalEmail: 1,email:1, name: 1, status: 1,  designation: 1, _id: 1, contact:1, password:1 }); // Include _id
